  ...
  - type: ContainerResource
    containerResource:
      ...
      target:
        ...
        averageValue: 30Mi
```
I applyied my HPA manifest to my Kubernetes instance. And time to check how it works.<br>
My application consumes 20Mb of RAM and only a little bit of CPU (when I looking on it by 'kubectl get hpa web-test-app-hpa --watch' there is an '<unknown>' status by CPU and a quite litte numbers in dashboard, so I just think that my application CPU consumption is about zero). I have no good idea how to load this application because it's quite simple. I had tryied to create load by 'kubectl run -i --tty load-generator --rm --image=busybox:1.28 --restart=Never -- /bin/sh -c "while sleep 0.01; do wget -q -O- http://web-test-app-service ; done"' but it's just consume a little bit of CPU, but not increasing RAM consumption. So I just set memory target to 15 Mb. Reapplyied HPA manifest and I can see that's working - now we have three replicas:<br>
![image](https://github.com/Slonskiy/slotegrator-test/assets/101737363/c07bf866-5796-4143-819e-671fba801f85)

Conclusion:<br>
It was my first time working wit Horizontal Pod Autoscaler, I have tryied moustly basic functions, but it was not too hard and now I see some examples how HPA can be configured.
