...
And, there is a great article about deploying statefull application into kubernetes from official [documentation](https://kubernetes.io/docs/tutorials/stateful-application/mysql-wordpress-persistent-volume/).<br>
...
First, ensure I'm in the context of your Minikube cluster:<br>
...
It will not work as minimum because I need to set 'imagePullPolicy: Never' due my environment, but I don't found the way how to do it with 'kubectl create deployment'. But it generated a draft that I needed (or I can just take it from any other deployment example).<br>
...
So, my draft in current moment looks like:<br>
```
apiVersion: apps/v1
kind: Deployment
metadata:
...
But in this case I will take it from examples and modify for my application:<br>
```
apiVersion: v1
kind: Service
metadata:
...
For some reasons (I don't want to use IP without name, and would like to look smarter) I added follow record to my '/etc/hosts' file:<br>
...
Conclusion:<br>
I can't say that I'm such expirienced Kubernetes user, but there are many possibilities to get required manifests or helm charts drafts, so I don't think that deploying of simple application to the kubernetes is too hard. Of course, Kubernetes not only about deploying applications. It's about scalability, it's about building images, it's about etcd and many-many more. But I hope, that I finished a program 'minimum' successfully.<br>

# Exercise 3

Setup HPA for our application.<br>
Actually, I did't anything like that before, so I just red official documentation ([this](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale-walkthrough/) and [this](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/#support-for-multiple-metrics)) and simple HPA can be generated by kubectl:<br>
`kubectl autoscale deployment web-test-app-deployment -n default --cpu-percent=20 --min=1 --max=3`<br>
As result I have a draft for my HPA:<br>
```
...
spec:
  ...
  - resource:
    ...
    type: Resource
```
I was changed some parameters of HPA for my application and in total I got:<br>
```
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  ...
  - type: ContainerResource
    containerResource:
      ...
      target:
        ...
        averageValue: 30Mi
```
I applyied my HPA manifest to my Kubernetes instance. And now is time to check how it works.<br>
My application consumes around 20Mb of RAM and only a little bit of CPU (when I looking on it by 'kubectl get hpa web-test-app-hpa --watch' there is an '<unknown>' status by CPU and a quite litte numbers in the cluster's dashboard, so I just think that my application CPU consumption is about zero). I have no good idea how to load this application because it's quite simple. I had tryied to create load by 'kubectl run -i --tty load-generator --rm --image=busybox:1.28 --restart=Never -- /bin/sh -c "while sleep 0.01; do wget -q -O- http://web-test-app-service ; done"' but it's just consume a little bit of CPU, but not increasing RAM consumption. So I just set memory target to 15 Mb. Reapplyied HPA manifest and I can see that's working - now we have three replicas:<br>
![image](https://github.com/Slonskiy/slotegrator-test/assets/101737363/c07bf866-5796-4143-819e-671fba801f85)

Conclusion:<br>
It was my first time working with Horizontal Pod Autoscaler, I have tryied moustly basic functions, but it was not too hard and now I see some examples how HPA can be configured.
